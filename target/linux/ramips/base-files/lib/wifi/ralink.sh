...
#	echo "Found "$((num-=1))"SSID Configure!"
    ...
    #by lintel: fix force ht40 in 11n 
    if [ "$ht" = "20" ]; then
    HT=0 
    elif [ "$ht" = "20+40" ]; then
    HT=1 
...
WscConfStatus=1
WCNTest=0
AccessPolicy0=${ra_macpolicy:-0}
AccessControlList0=${maclist:-0}
AccessPolicy1=0
AccessControlList1=
AccessPolicy2=0
AccessControlList2=
...
rt305x_fix_ioctl() {
	...
	if [ $2 == 1 ]; then
		if [ -n "${RT3X5X}" ]; then
		echo "SoC is RT305x,disable bridge !"
...
#	echo  "vifs="$vifs
	...
	[ "$disabled" = "1" ] && return
	for vif in $vifs; do
		local ifname encryption key ssid mode cliname
		config_get ifname $vif ifname
		config_get cliname $vif cliname
...
#   list maclist '7C:DD:90:22:89:FA'
...
config wifi-iface
	...
	option network	lan
	option mode     ap
	option ssid     xRouter${i#0}_$(cat /sys/class/net/ra${i}/address|awk -F ":" '{print $4""$5""$6 }'| tr a-z A-Z)
